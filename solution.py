...
def naked_twins(values):
    ...
    for unit in unitlist:
        unitValue = [values[box] for box in unit if len(values[box]) == 2]
        for value in unitValue:
            naked_twin = set(k for k in unitValue if unitValue.count(k) == 2)
...
def only_choice(values):
    ...
    for unit in unitlist:
        for digit in '123456789':
            temp = [box for box in unit if digit in values[box]]
            if len(temp) == 1:
                values = assign_value(values,temp[0], digit)
    return values
...
def search(values):
    "Using depth-first search to try all possible values."
